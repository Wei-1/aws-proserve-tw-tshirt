...
def getS3Object(bucket_name, key):
    print("Loading file: ", bucket_name, key)
...
def lambda_handler(event, context):
    ...
    res = [getS3Object(bucket_name, key) for key in file_list if not key.endswith('/')]
    ...
    for r in res:
        ...
        if user not in timemap or timemap[user] < surveytime:
            ...
            latestobj[user] = r['data'][0]['data']
    
    saveFile('lake-ingest-621149070359', 'tshirt/vote/result.json', json.dumps(latestobj).encode("utf-8"))
